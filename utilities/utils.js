...
async function updateReferralLinks(memberId, referralLinkA, referralLinkB) {
    try {
        await query(
            'UPDATE `fx-users` SET referral_link_a = ?, referral_link_b = ? WHERE member_id = ?',
            [referralLinkA, referralLinkB, memberId]
        );
    } catch (error) {
        console.error('Error in updateReferralLinks:', error);
        throw error;
    }
}
