...
// Stripe payment endpoint
router.get('/get-withdrawals/:userId', isAuthenticated, async (req, res) => {
	...
	try {
		...
		if (req.locals.id !== userId)
			return res.status(401).json({ success: false, message: 'You cannot access withdrawal data other than yours' });

		const withdrawals = await db.query(
			'SELECT amount, status, date, wallet_address FROM withdrawals WHERE member_id = ?',
			[userId]
		);

		res.json({ success: true, withdrawals });
	} catch (error) {
		logger.error(`Failed to get withdrawal results of user ${userId}:`, error);
